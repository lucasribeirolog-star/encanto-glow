...
(function () {
  ...
  // Versão simples de um procedimento cadastrado só com o básico pelo painel
  // (nome/ícone/resumo), ainda sem o conteúdo completo (benefícios, etapas,
  // FAQ etc.) — usada até esse conteúdo ser adicionado ao site.
  function renderTreatmentBasico(basico) {
    document.title = basico.name + " em Sorocaba/SP — Encanto Glow | Dra. Laureen Polidoro";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.setAttribute("content", (basico.short || basico.name) + " Clínica de estética em Sorocaba/SP, com a Dra. Laureen Polidoro.");

    root.innerHTML = `
      <section class="tmt-hero">
        <div class="container tmt-hero-inner">
          <a href="index.html#servicos" class="tmt-back">&larr; Todos os tratamentos</a>
          <div class="tmt-icon">${basico.icon}</div>
      ...
      <section class="tmt-content">
        <div class="container tmt-grid">
          <div class="tmt-main">
            <div class="tmt-block">
              <h2>Sobre esse procedimento</h2>
              <p>${basico.short || "Em breve, mais detalhes sobre esse procedimento."} Fale com a gente pra saber mais.</p>
            </div>
          </div>
          <aside class="tmt-side">
            <div class="tmt-side-card tmt-side-cta">
              <h3>Quer saber mais?</h3>
              <p>Fale direto com a Dra. Laureen Polidoro e tire todas as suas dúvidas sobre esse procedimento.</p>
              <a href="index.html#contato" class="btn btn-primary" style="width:100%; justify-content:center;">Agendar avaliação</a>
              <a href="https://wa.me/5515992809088" target="_blank" rel="noopener" class="btn btn-ghost" style="width:100%; justify-content:center; margin-top:0.7rem;">Falar no WhatsApp</a>
            </div>
          </aside>
        </div>
      </section>`;
  ...
  function renderTreatment(treatment) {
    document.title = treatment.name + " em Sorocaba/SP — Encanto Glow | Dra. Laureen Polidoro";
    ...
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.setAttribute("content", treatment.short + " Clínica de estética em Sorocaba/SP, com a Dra. Laureen Polidoro.");
  ...
  if (treatment) {
    ...
    root.innerHTML = `
      <section class="tmt-hero">
        <div class="container tmt-hero-inner">
          <p style="color:var(--white);">Carregando...</p>
        </div>
      </section>`;
  }

  // Busca o básico cadastrado no painel (nome/ícone/resumo). Se o
  // procedimento já tinha conteúdo completo, só atualiza esses campos sem
  // refazer a página inteira (evita qualquer soluço visual). Se não existia
  // (procedimento novo, cadastrado só com o básico), mostra uma versão
  // simples até o conteúdo completo ser adicionado.
  fetch(`${WEBHOOK_URL}?action=list_procedimentos`)
    .then((res) => res.json())
    .then((json) => {
      const row = (json.rows || []).find((r) => r["ID"] === slug);
      if (treatment) {
        if (!row) return;
        treatment.name = row["Nome"] || treatment.name;
        treatment.icon = row["Ícone"] || treatment.icon;
        treatment.short = row["Resumo"] || treatment.short;
        document.title = treatment.name + " em Sorocaba/SP — Encanto Glow | Dra. Laureen Polidoro";
        const metaDesc = document.querySelector('meta[name="description"]');
        if (metaDesc) metaDesc.setAttribute("content", treatment.short + " Clínica de estética em Sorocaba/SP, com a Dra. Laureen Polidoro.");
        const h1 = root.querySelector(".tmt-hero-inner h1");
        const p = root.querySelector(".tmt-hero-inner p");
        const iconEl = root.querySelector(".tmt-icon");
        if (h1) h1.textContent = treatment.name;
        if (p) p.textContent = treatment.short;
        if (iconEl) iconEl.textContent = treatment.icon;
      } else if (row) {
        renderTreatmentBasico({ slug, name: row["Nome"] || "", icon: row["Ícone"] || "✨", short: row["Resumo"] || "" });
      } else {
        renderNotFound();
      }
    })
    .catch(() => {
      if (!treatment) renderNotFound();
    });
})();
